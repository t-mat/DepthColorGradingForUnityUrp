...
namespace DepthColorGrading
{
    public sealed class DepthColorGradingLut2d
    {
        ...
        private static void UpdateLutTexture2D(Texture2D tex2d, float dh, float ds, float dl, float dgamma, Color tint, Color lift)
        {
            dh = Mathf.Pow(dh, 3f);
            ...
            FillLutTexture2D(tex2d, filter: (linearColor) => {
                float gr = Mathf.Pow(linearColor.r, gm);
                float gg = Mathf.Pow(linearColor.g, gm);
                float gb = Mathf.Pow(linearColor.b, gm);
                ...
                float r = Mathf.Lerp(newRgb.x, newRgb.x * tint.r, tint.a) + lift.r * lift.a;
                float g = Mathf.Lerp(newRgb.y, newRgb.y * tint.g, tint.a) + lift.g * lift.a;
                float b = Mathf.Lerp(newRgb.z, newRgb.z * tint.b, tint.a) + lift.b * lift.a;
                return new Color(r, g, b, linearColor.a);
            });
        }

        private static void FillLutTexture2D(Texture2D tex2d, System.Func<Color,Color> filter)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    int r = x % size;                       // r:[0 , size-1]
                    int b = x / size;                       // b:[0 , size-1]
                    var s = new Color(r*d, g*d, b*d, 1);    // s:[0.0 , 1.0]
                    colors[y * width + x] = filter(s);
                }
            }
